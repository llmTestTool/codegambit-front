...
const ApplicantCardList: React.FC<ApplicantCardListProps> = ({ applicants }) => {
  ...
  return (
    <div className="lg:hidden">
      {applicants.map((applicant) => (
        <div key={applicant.id} className="border-b border-gray-200 px-4 py-4 sm:px-6">
          <div className="flex items-center justify-between">
            <div className="flex-1 min-w-0">
              <h3 className="text-sm font-medium text-gray-900 truncate">{applicant.name}</h3>
              <p className="text-sm text-gray-500 truncate">{applicant.assignmentTitle}</p>
            </div>
